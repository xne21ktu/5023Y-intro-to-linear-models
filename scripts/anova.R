...
# The fourth column is the mean sum of squares ( average variability per treatment level), the first row is MSR (mean sum of square residuals) and the second row is te MSE (mean sum of squared error). MSR = SSR/(k-1) & MSE = SSE/(N-k).
...
# Two-Way ANOVA ----
# includes two explanatory variables. Usually these are treatments of interest, but for this example we will stick with including the pair variable.

lsmodel2 <- lm(height ~ type + as.factor(pair), data = darwin)

anova(lsmodel2)
# Can see some of the degrees of freedom and SSE which were initially in the residuals, have now been 'used' by the new pair term (in the form of SSR) in the more complex model
# Pairing has actually increased the relative proportion of SSE compared to SSR (implies a 'negative variance component').
# When mean squares of the regression (6.2) are smaller than the residuals (11.1) it implies a problem with the experimental design. It could be a result of undersampling (more pairs are needed), however it could also be that these pairs were not sampled at random.